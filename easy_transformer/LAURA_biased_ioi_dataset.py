"""
biased_ioi_dataset.py

A biased variant of the IOI dataset for studying name-order vs. role asymmetries.
...
Both groups use the same family of "When [IO] and [S] went to the [PLACE],
[S] gave a [OBJECT] to [IO]" templates.  Only the [PLACE] and [OBJECT] slots
vary across prompts.
...
switch_perc  (float, 0–1)
    ...
    only, i.e.
        normal : "When [IO] and [S] went to the [PLACE], [S] gave a [OBJECT] to [IO]"
        switched: "When [S]  and [IO] went to the [PLACE], [S] gave a [OBJECT] to [IO]"
    The IO/S labels are NOT changed — only the surface word order in the
    introductory clause flips.  This lets you tease apart positional vs.
    structural cues.
...
# These can be overridden per-instance via the io_name / s_name parameters.
# IMPORTANT: both names must tokenize to a SINGLE GPT-2 token (as " Name").
# Safe single-token names include: Mary, John, Alice, Bob, Sarah, James,
# Emma, Tom, Laura, Anna, Kate, Mark, Paul, Lisa, Ruth, Amy, Luke.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Templates
# Standard form: "When [IO] and [S] went to the [PLACE], [S] gave a [OBJECT] to [IO]"
# All placeholders:  [IO], [S], [PLACE], [OBJECT]
# ---------------------------------------------------------------------------
BIASED_TEMPLATES = [
...
# The second clause ("[S] gave … to [IO]") stays identical.
...
def gen_biased_prompts(
    ...
    """Generate a list of biased IOI prompt dicts.
    ...
    -------
    list of dict
        Each dict has keys: "text", "IO", "S", "TEMPLATE_IDX",
        "[PLACE]", "[OBJECT]", "switched".
    """
    if seed is not None:
        random.seed(seed)
...
class BiasedIOIDataset:
    """Biased IOI dataset with fixed names (Mary / John) and 70/30 role split.
    ...
    switch_perc : float
        ...
        swapped while IO/S labels remain unchanged.  Adjust this to test
        whether the model relies on name position vs. sentence structure.
    bias_ratio : float
    ...
    # expose the switch percentage so callers can read it back
    switch_perc: float = 0.0
