"""
LAURA_full_experiment_extra.py

Unified experiment runner using the EXTRA biased IOI dataset (prompts include
an intermediate sentence naming the IO between the introductory clause and
the giving clause, e.g.:
    "When Mary and John went to the store, Mary had a good day.
     John gave a drink to Mary"

Runs the full pipeline (logits → macro states → EI / Eff / ΔEff) for three
model conditions, all sharing the same dataset:
    ...
    3. Untrained GPT-2     — same architecture, weights re-initialised
...
HOW TO RUN
----------
    python LAURA_full_experiment_extra.py > full_experiment_extra.out 2>progress.err
    stdout → full_experiment_extra.out   (all results, tables, per-prompt detail)
...
numb        = 1000000   # total number of prompts (shared across all conditions)
top_n       = 10     # top-K tokens kept at the micro level
bias_ratio  = 0.5    # fraction of prompts where NAME_MAJORITY is IO
switch_perc = 0.1    # fraction of prompts with swapped first-clause order
...
from easy_transformer.LAURA_biased_ioi_dataset_extra import BiasedIOIDataset
...
rule()
print("  Biased IOI Experiment (Extra dataset)  —  Effective Information Analysis")
rule()
print(f"""
  Dataset      numb={numb}  bias={bias_ratio:.0%} {NAME_MAJORITY}=IO  switch={switch_perc:.0%}  seed={seed}
               Extra variant: intermediate sentence naming IO inserted between
               intro clause and giving clause.
...
print(f"  Untrained model seed: {untrained_seed}", flush=True)
...
with torch.no_grad():
    for name, param in model_untrained.named_parameters():
        ...
        if last == "w" and param.dim() == 1:
            param.fill_(1.0)       # LayerNorm scale → neutral
        elif last == "b" and param.dim() == 1:
            param.zero_()          # LayerNorm bias  → neutral
        else:
            param.normal_(0.0, 0.02)   # all weights fully randomised
